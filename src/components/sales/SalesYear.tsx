// src/components/sales/SalesYear.tsx
import { Card } from "@/components/ui/card";
import { DollarSign } from "lucide-react";
import { Sale, PaymentMethod } from "./types";

interface SalesYearProps {
  sales: Sale[];
...
export function SalesYear({ sales, paymentMethods, formatCurrency, isThisYear }: SalesYearProps) {
  return (
    <Card className="p-6">
      <h3 className="text-lg font-semibold mb-4 flex items-center space-x-2">
        <DollarSign className="w-5 h-5" />
        <span>Vendas por Forma de Pagamento (Ano)</span>
      </h3>
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {paymentMethods.map((method) => {
          const yearSales = sales.filter(
            (s) => isThisYear(s.date) && s.paymentMethod === method.type
          );
