// src/components/sales/SalesMonth.tsx
import { Card } from "@/components/ui/card";
import { DollarSign } from "lucide-react";
import { Sale, PaymentMethod } from "./types";

interface SalesMonthProps {
  sales: Sale[];
...
export function SalesMonth({ sales, paymentMethods, formatCurrency, isThisMonth }: SalesMonthProps) {
  return (
    <Card className="p-6">
      <h3 className="text-lg font-semibold mb-4 flex items-center space-x-2">
        <DollarSign className="w-5 h-5" />
        <span>Vendas por Forma de Pagamento (Mês)</span>
      </h3>
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {paymentMethods.map((method) => {
          const monthSales = sales.filter(
            (s) => isThisMonth(s.date) && s.paymentMethod === method.type
          );
